...
import { MessageSquare, Send, X, Bot, Sparkles, Calendar, Target, BookOpen, RefreshCw, CheckCircle2, KeyRound, AlertTriangle } from 'lucide-react';
...
export const AIChatBox: React.FC<AIChatBoxProps> = ({ 
  ...
  const [nvidiaApiKey, setNvidiaApiKey] = useState(() => localStorage.getItem('examscmd_nvidia_api_key') || '');
  ...
  useEffect(() => {
    localStorage.setItem('examscmd_nvidia_api_key', nvidiaApiKey.trim());
  }, [nvidiaApiKey]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorPayload.error || 'Failed to fetch from AI Function');
      }
        ...
        {isOpen && (
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={(e) => { e.stopPropagation(); setShowApiKeySettings(prev => !prev); }}
                  className="p-1 text-white/20 hover:text-amber-300 transition-colors"
                  title="NVIDIA key settings"
                >
              ...
              <>
                {showApiKeySettings && (
                  <div className="px-4 py-3 border-b border-white/5 bg-amber-500/5">
                    <label className="block text-[9px] font-bold text-amber-300 uppercase tracking-widest mb-2">
                      NVIDIA API Key Fallback
                    </label>
                    <input
                      type="password"
                      value={nvidiaApiKey}
                      ...
                      placeholder="nvapi-..."
                      className="w-full bg-white/5 border border-white/10 rounded-lg py-2 px-3 text-xs text-white placeholder:text-white/30 focus:outline-none focus:border-amber-300/40"
                    />
                    <p className="text-[9px] text-white/40 mt-2">
                      Used only when server-side key is missing.
                    </p>
                  </div>
                )}
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-hide bg-black/20">
                  {apiError && (
                    <div className="bg-red-500/10 border border-red-500/30 text-red-200 rounded-xl p-3 flex items-start gap-2">
                      <AlertTriangle className="w-3.5 h-3.5 mt-0.5 shrink-0" />
                      <div>
                        <p className="text-[10px] font-semibold uppercase tracking-wider">API Error</p>
